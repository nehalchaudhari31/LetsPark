...
<script data-jsd-embedded data-key="681cc4c1-0c35-44d3-a1fe-1691537530fc" data-base-url="https://jsd-widget.atlassian.com" src="https://jsd-widget.atlassian.com/assets/embed.js"></script>
	...
	String url = "jdbc:mysql://localhost:3306/login?characterEncoding=latin1";
			...
			String query = "select * from mallsentry ";
	...
	try{
		...
		<table>
		<tr><th>Username</th> <th>Name</th> <th>Email</th> <th>Wheeler</th> <th>Slots Booked</th> <th>From Time</th> <th>To Time</th> <th>Mall</th> <th>Alloted Parking No.</th> <th>Cost</th>    </tr>
		<%
		do{
			%>
			<tr><td><%=rs.getString(1)%></td> <td><%=rs.getString(2) %></td> <td><%=rs.getString(3) %></td> <td><%=rs.getString(4) %></td> <td><%=rs.getString(5) %></td> <td><%=rs.getString(6) %></td> <td><%=rs.getString(7) %></td> <td><%=rs.getString(8) %></td>   <td><%=rs.getString(9) %></td>  <td><%=rs.getString(10) %></td>   </tr> 
			<%
